...
export default function QuoteGroupDetailPage() {
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      try {
        ...
        setData(payload as GroupResponse);
      } catch (e) {
        if (!alive) return;
        console.error("quote group error:", e);
  ...
  const fmtMoney = (v?: number) => `L ${money.format(Number(v ?? 0))}`;
  ...
  const totalEnLetras = useMemo(() => {
    const value = Number(data?.group?.total ?? 0);
    const unidades = [
      "CERO",
      "UNO",
      "DOS",
  ...
  const linesByItem = useMemo(() => {
    const map = new Map<string, GroupLine[]>();
    for (const line of data?.lines ?? []) {
      const arr = map.get(line.group_item_id) ?? [];
      arr.push(line);
      map.set(line.group_item_id, arr);
    }
    return map;
  }, [data?.lines]);
    ...
    <RequireAuth>
      <>
        <AppShell
          ...
          subtitle="Vista tipo hoja para múltiples productos."
          crumbs={[
            { label: "Inicio", href: "/" },
            { label: "Cotización" },
            { label: "Detalle" },
        ...
        >
          ...
          <div className="relative space-y-6">
            <div className="pointer-events-none absolute -top-20 right-6 h-64 w-64 rounded-full bg-[#38BDF8]/15 blur-3xl no-print" />
            <div className="pointer-events-none absolute top-40 -left-16 h-72 w-72 rounded-full bg-[#22C55E]/10 blur-3xl no-print" />

            {loading ? (
              <Card className="p-6">Cargando...</Card>
            ) : !data ? (
              <Card className="p-6">No se pudo cargar la cotización.</Card>
            ) : (
                      ...
                      <div className="a4-content relative text-[11px]">
                        <div className="flex items-start justify-between gap-4">
                          ...
                          <div className="text-right">
                            <div className="text-[10px] uppercase tracking-[0.35em] text-[#64748B]">
                              Cotización interna
                            </div>
                            <div className="text-base font-semibold uppercase">Factura</div>
                            <div className="text-xs text-[#475569]">
                              No. {data.group.id.slice(0, 12).toUpperCase()}
                            </div>
                            <div className="mt-2 inline-flex items-center gap-2 rounded-full border border-[#CBD5F5] px-3 py-1 text-[11px]">
                              ...
                              <Badge variant="neutral">{data.group.status}</Badge>
                            </div>
                            <div className="mt-2 text-[11px] text-[#475569]">
                              Fecha: {fmtDate(data.group.created_at)}
                            </div>
                          </div>
                        </div>
                        ...
                        <div className="mt-3 rounded-lg border border-[#E2E8F0] bg-[#F8FAFC] px-3 py-2 text-[11px]">
                          <div className="grid grid-cols-[100px_1fr_90px_1fr] gap-2">
                            ...
                            <div className="border-b border-[#CBD5F5]">
                              {data.customer?.name ?? "—"}
                            </div>
                          ...
                          <div className="mt-2 grid grid-cols-[100px_1fr_90px_1fr] gap-2">
                            <div className="font-semibold">DIRECCIÓN:</div>
                            <div className="border-b border-[#CBD5F5]">
                              {data.customer?.address ?? "—"}
                            </div>
                            <div className="font-semibold text-right">TEL / CORREO:</div>
                            <div className="border-b border-[#CBD5F5]">
                              {data.customer?.phone ?? "—"}{" "}
                        ...
                        <div className="mt-3 overflow-hidden rounded-lg border border-[#CBD5F5]">
                          <div className="grid grid-cols-[90px_1fr_120px_150px_110px] bg-[#1F2937] text-[10px] font-semibold text-white">
                            <div className="px-2 py-1.5 text-center">CANTIDAD</div>
                            <div className="px-2 py-1.5 text-center">DESCRIPCIÓN</div>
                            <div className="px-2 py-1.5 text-center">PRECIO UNITARIO</div>
                            <div className="px-2 py-1.5 text-center">DESCUENTOS</div>
                            <div className="px-2 py-1.5 text-center">TOTAL</div>
                          </div>
                          <div className="bg-[#D9F0FF] text-[11px] text-[#0F172A]">
                            {data.items.map((item) => {
                              const qty = Number(item.inputs?.cantidad ?? 1);
                              const unitPrice = qty > 0 ? item.price_final / qty : item.price_final;
                              const discount = item.discount_pct ? (item.price_final * item.discount_pct) / 100 : 0;
                              return (
                                <div
                                  key={item.id}
                                  className="grid grid-cols-[90px_1fr_120px_150px_110px] border-b border-[#CBD5F5]"
                                >
                                  ...
                                  <div className="px-2 py-2">
                                    ...
                                    <div className="text-[10px] text-[#475569]">
                                      Diseño: {item.inputs?.diseño ?? "-"}
                                    </div>
                                  </div>
                                  <div className="px-2 py-2 text-right">{fmtMoney(unitPrice)}</div>
                                  <div className="px-2 py-2 text-right">{fmtMoney(discount)}</div>
                                  <div className="px-2 py-2 text-right">{fmtMoney(item.price_final)}</div>
                                </div>
                              );
                            })}
                          </div>
                          <div className="grid grid-cols-[90px_1fr_120px_150px_110px] border-t border-[#CBD5F5] bg-[#CDE9FF] text-[11px] font-semibold">
                            ...
                            <div className="px-2 py-2 text-right">{fmtMoney(data.group.price_final)}</div>
                          </div>
                        </div>
                ...
                {data.items.map((item) => (
                  <Card key={item.id} className="p-6">
                    <div className="text-xs uppercase tracking-[0.2em] text-[#94A3B8]">
                      Insumos (interno) — {item.product?.name ?? item.product_id}
                    </div>
                    <div className="mt-3 overflow-hidden rounded-xl border border-[#334155] bg-[#0F172A]/80">
                      <table className="w-full text-xs text-[#E2E8F0]">
                        ...
                        <tbody className="divide-y divide-[#334155]">
                          {(linesByItem.get(item.id) ?? []).length === 0 ? (
                            <tr>
                            ...
                            (linesByItem.get(item.id) ?? []).map((line) => (
                              <tr key={`${item.id}-${line.supply_id}`}>
                                <td className="px-3 py-2">{line.supply_name}</td>
                                <td className="px-3 py-2">{line.unit_base}</td>
                                <td className="px-3 py-2 text-right">
                                  {money.format(Number(line.qty ?? 0))}
                                </td>
                                <td className="px-3 py-2 text-right">
                                  {fmtMoney(line.cost_per_unit)}
                                </td>
                                <td className="px-3 py-2 text-right">
